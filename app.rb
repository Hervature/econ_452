...
(1..29).each do |page|
  puts page
  # Manually change year and season and playoff and preseason
  @session.visit "http://www.oddsportal.com/hockey/usa/nhl-2014-2015/results/#/page/#{page}/"
  sleep(2)
  ...
  @html_doc.css('tr.deactivate').each do |tr|
    ...
    if @date.strftime('%Y').to_i == 2015 && (@date.strftime('%m').to_i > 4 || (@date.strftime('%m').to_i == 4 && @date.strftime('%d').to_i > 13)) then
      @playoff_game = true
    elsif @date.strftime('%Y').to_i == 2014 && (@date.strftime('%m').to_i < 10 || (@date.strftime('%m').to_i == 10 && @date.strftime('%d').to_i < 6)) then
      @preseason_game = true
    end
